<?php 
	...
	if((isset($_SESSION['id']) || isset($_GET['user_identity'])) && !isset($_POST['updateProfileBtn'])){
		...
		if(empty($form_errors)){
			try{
				//create sql update
				$sqlUpdate = "UPDATE users SET username =:username, email =:email, steamid =:steamid WHERE id =:id";

				//Sanitise 
				$statement = $db->prepare($sqlUpdate);

				//update 
				$statement->execute(array(':username' => $username, ':email' => $email, ':steamid' => $steamid, ':id' => $hidden_id));

				//Check if one new row was created
				if($statement->rowCount() == 1){
					$result = '
						<script type="text/javascript">
							swal("Updated!", "Profil mise à jour", "Valider");
						</script>
					';
				}
				else {
